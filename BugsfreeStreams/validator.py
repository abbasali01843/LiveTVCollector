...
TIMEOUT = 6
WORKERS = 32
HEADERS = {"User-Agent": "LiveTVCollector-Health/1.4"}
...
def protocol(url: str, content_type: str = "") -> str:
    ...
    if value.endswith(".m3u8") or "mpegurl" in ct or "vnd.apple.mpegurl" in ct: return "hls"
    if value.endswith(".mpd") or "dash+xml" in ct: return "dash"
    if value.endswith((".mp4", ".ts", ".mkv", ".webm", ".avi", ".flv")): return "media"
    return "unknown"
...
def score(status: str, proto: str, redirected: bool = False, segment_ok: bool | None = None) -> int:
    if status == "active":
        ...
        if segment_ok is False: value -= 35
...
def probe(channel: dict) -> dict:
    ...
    result = {"name": channel.get("name", "Unnamed Channel"), "url": url,
    ...
    try:
        ...
        result.update(http_status=r.status_code, final_url=normalize_url(r.url), redirected=normalize_url(r.url) != url,
                      protocol=protocol(r.url, r.headers.get("content-type", "")))
        if r.status_code in (401, 403, 451): result["status"] = "geo_or_restricted"; result["score"] = score(result["status"], result["protocol"]); return result
        if 200 <= r.status_code < 400 and result["protocol"] not in {"hls", "dash"}:
            result["status"] = "active"; result["score"] = score("active", result["protocol"], result["redirected"]); return result
    except requests.Timeout:
        result["status"] = "timeout"; result["score"] = score("timeout", result["protocol"]); return result
    except requests.RequestException: pass
    try:
        with requests.get(url, headers=HEADERS, timeout=TIMEOUT, allow_redirects=True, stream=True) as r:
            result.update(http_status=r.status_code, final_url=normalize_url(r.url), redirected=normalize_url(r.url) != url,
                          protocol=protocol(r.url, r.headers.get("content-type", "")))
            if r.status_code in (401, 403, 451): result["status"] = "geo_or_restricted"
            elif 200 <= r.status_code < 400:
                if result["protocol"] == "hls":
                    ok, segment = probe_hls(r); result.update(manifest_ok=ok, sample_segment=segment)
                    ...
                    chunk = next(r.iter_content(chunk_size=16384), b""); result["manifest_ok"] = b"<MPD" in chunk or b":MPD" in chunk
    ...
    except requests.RequestException: result["status"] = "down"
...
def write_app_exports(results: list[dict], checked_at: str) -> None:
    ...
    (OUT / "active.json").write_text(json.dumps({"updated": checked_at, "count": len(active), "channels": active}, ensure_ascii=False, indent=2), encoding="utf-8")
    with (OUT / "active.m3u").open("w", encoding="utf-8") as f:
        f.write("#EXTM3U\n")
        for ch in active:
            f.write(f'#EXTINF:-1 tvg-logo="{ch.get("logo", "")}" group-title="{ch.get("group", "Uncategorized")}",{ch.get("name", "Unnamed Channel")}\n{ch["final_url"]}\n')
    by_country = defaultdict(list)
    for item in results: by_country[item.get("country", "Unknown")].append(item)
    countries = {}
    for country, items in sorted(by_country.items()):
        counts = defaultdict(int)
        for item in items: counts[item.get("status", "unknown")] += 1
        countries[country] = {"total": len(items), "active": counts["active"], "geo_or_restricted": counts["geo_or_restricted"],
    ...
    (OUT / "countries.json").write_text(json.dumps({"updated": checked_at, "countries": countries}, ensure_ascii=False, indent=2), encoding="utf-8")
...
def main() -> None:
    ...
    for channel in load_channels():
        url = normalize_url(channel.get("url", ""))
        if url:
            item = dict(channel); item["url"] = url; unique.setdefault(url, item)
    with concurrent.futures.ThreadPoolExecutor(max_workers=WORKERS) as pool:
        results = list(pool.map(probe, unique.values()))
    checked_at = datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    summary = defaultdict(int)
    for item in results: summary[item["status"]] += 1
    payload = {"updated": checked_at, "total_unique_streams": len(results), "summary": dict(summary),
               "active_streams": summary["active"], "average_score": round(sum(x["score"] for x in results) / len(results), 2) if results else 0,
               "streams": results}
    OUT.mkdir(parents=True, exist_ok=True)
